...
```
#### 多个toast出现的时候，只留下一个toast
* 重构plugin.js里面的代码，把它写成功一个函数createToast,然后确认使用了哪些参数。如果有toast，就关闭这个toast。这样就没有重复的，值留下一个toast了。
```
...
```
* 我们通过浮点内置函数[Math.random()](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Math/random)来测试就可以知道每次产生的数值都不同了
```
    methods:{
        showToast(){
